...
bool Board :: operator== (const Board& other_in) const
{
	assert(isInvariantTrue());
	assert(other_in.isInvariantTrue());

	if(board_size != other_in.board_size)
		return false;
	if(place_count != other_in.place_count)
		return false;
	for(int i = 0; i < place_count; i++)
	{
		if(p_places[i] != other_in.p_places[i])
			return false;
	}
	return true;
}
